...
export default function MainDashboardScreen({ onNavigate, lang, onQuizNavigate, lessons = [] }: MainDashboardScreenProps) {
  ...
  const handleShareClick = async () => {
    const shareData = {
      title: lang === 'ar' ? 'تطبيق الأحياء الذكي' : 'Smart Biology App',
      text: lang === 'ar' 
        ? 'حمل تطبيق الأحياء الذكي للثانوية العامة واستمتع بالملخصات والخرائط والكويزات التفاعلية مجاناً!' 
    ...
    if (navigator.share) {
      ...
      try {
        ...
        alert(lang === 'ar' ? 'تم نسخ رابط المشاركة إلى الحافظة! 🚀' : 'Share link copied to clipboard! 🚀');
      } catch (err) {
  ...
  const handleWhatsappClick = () => {
    const phoneNumber = '967777777777'; // Placeholder
    const text = encodeURIComponent(
      lang === 'ar' 
        ? 'مرحباً، أود الاستفسار عن تطبيق الأحياء وتفعيل الباقة الكاملة.' 
        : 'Hello, I would like to inquire about the biology app and activating the premium package.'
    );
